...
class DataCollatorForWholeWordMask(DataCollatorForLanguageModeling):
    ...
    def __init__(self, tokenizer, mlm, mlm_probability, masking_list):
        tokenizer = tokenizer
        mlm = mlm
        ...
        print('Preparing...')
        self.numeric_change_related_words_to_tokens = {masking_word:tokenizer.convert_ids_to_tokens(tokenizer(masking_word)['input_ids'])[1:-1] for masking_word in masking_list}
    ...
    def _whole_word_mask(self, input_tokens: List[str], max_predictions=512):
        ...
        masking_list_selected = list(filter(lambda x: x.lower() in text.lower(), self.masking_list))

        for name in masking_list_selected: 
            range_list_of_indices.extend(find_sub_list_indices(self.numeric_change_related_words_to_tokens[name], input_tokens))
        ...
        # 증감 관련 단어 토큰들을 제외한 토큰들
        cand_indexes = []
        for idx in range(1, len(input_tokens)-1):
