...
class FeatureExtractor_cls_embedding:
    ...
    def get_cls_embedding(self, text):
        ...
        cls_feature = self.model(input_ids, attention_mask=attention_mask, return_dict=True, output_hidden_states=True)['hidden_states'][0]
        return np.squeeze(cls_feature).mean(0).detach().cpu().numpy() 
